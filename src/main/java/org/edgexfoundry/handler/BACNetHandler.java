...
import org.edgexfoundry.bacnet.BACNetDriver;
import org.edgexfoundry.bacnet.DeviceDiscovery;
...
public class BACNetHandler {
	...
	private final static EdgeXLogger logger = EdgeXLoggerFactory.getEdgeXLogger(BACNetHandler.class);
	...
	@Value("${bacnet.device.init:#{null}}")
	private String bacnetInit;
	@Value("${bacnet.device.init.args:#{null}}")
	private String bacnetInitArgs;
	
	@Value("${bacnet.device.remove:#{null}}")
	private String bacnetRemove;
	@Value("${bacnet.device.remove.args:#{null}}")
	private String bacnetRemoveArgs;
	...
	public void initializeDevice(Device device) {
		BACNetDevice BACNetDevice = new BACNetDevice(device);
		BACNetDevice.setFailures(0);
		if(bacnetInit != null && commandExists(device, bacnetInit))
			executeCommand(BACNetDevice, bacnetInit, bacnetInitArgs);
		logger.info("Initialized Device: " + device.getName());
	}

	public void disconnectDevice(Device device) {
		BACNetDevice BACNetDevice = new BACNetDevice(device);
		if (bacnetRemove != null && commandExists(device, bacnetRemove))
			executeCommand(BACNetDevice, bacnetRemove, bacnetRemoveArgs);
	...
	public Map<String, String> executeCommand(BACNetDevice device, String cmd, String arguments) {
	...
	private Boolean requiresQuery(boolean immediate, String method, BACNetDevice device, ResourceOperation operation) {
	...
	private BACNetObject getBACNetObject(Map<String, BACNetObject> objects, String objectName, String transactionId) {
		BACNetObject object = objects.get(objectName);
