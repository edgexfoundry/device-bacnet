...
public class DeviceStore {
	...
	// cache for devices
	private Map<String, BACNetDevice> devices = new HashMap<String,BACNetDevice>();
	...
	public boolean remove(Device device) {
		...
		if (devices.containsKey(device.getName())) {
			...
			BACNet.disconnectDevice(device);
	...
	public boolean remove(String deviceId) {
		BACNetDevice d = devices.values().stream().filter(device -> device.getId().equals(deviceId)).findAny().orElse(null);
		if (d != null) {
			remove(d);
		}
		return true;
	...
	public boolean add(Device device) {
		...
		if (metaDevice.getOperatingState().equals(OperatingState.enabled))
			BACNet.initializeDevice(metaDevice);
		return true;
	}
	...
	private Device addDeviceToMetaData(Device device) {
		...
		devices.put(device.getName(), BACNetDevice);
		return device;
	}
	...
	public Map<String, BACNetDevice> getBACNetDevices() {
		return devices;
	}
	...
	public BACNetDevice getBACNetDeviceById(String deviceId) {
		if (devices != null) {
			return devices.values().stream().filter(device -> device.getId().equals(deviceId)).findAny().orElse(null);
		}
		return null;
